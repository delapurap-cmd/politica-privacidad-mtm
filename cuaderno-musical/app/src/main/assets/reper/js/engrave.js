...
const Engrave = (() => {
  ...
  function render(score, root, opts = {}) {
    ...
    pages.forEach((systems, pageIndex) => {
      const pageHeight = compact ? Math.max(218, 72 + systems.length * SYSTEM_H) : PAGE.h;
      const pageEl = document.createElement('div');
      pageEl.className = 'sheet';
      pageEl.style.aspectRatio = `${PAGE.w} / ${pageHeight}`;
      ...
      const top = compact ? 38 : (pageIndex === 0 ? M.topFirst : M.top);
