...
const ScoreBlocks = (() => {
  ...
  function normalizeScore(score) {
    if (!score || !Array.isArray(score.measures)) {
      return Model.newScore({ systems: 1, measuresPerSystem: 4, systemsPerPage: 8 });
    }
    const s = clone(score);
    s.key = s.key || 'C';
    s.time = s.time || { num: 4, den: 4 };
    s.tempo = s.tempo || 90;
    s.measuresPerSystem = s.measuresPerSystem || 4;
  ...
  function add(page, data = null, openEditor = true) {
    ...
    const item = {
      ...
      x: data && Number.isFinite(+data.x) ? +data.x : .08,
      y: data && Number.isFinite(+data.y) ? +data.y : Math.max(.18, visibleY),
      w: data && Number.isFinite(+data.w) ? +data.w : .84,
  ...
  function render(item) {
    ...
    try {
      Engrave.render(item.score, item.preview, { compact: true });
    } catch (e) {
